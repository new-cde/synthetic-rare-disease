...
def run():
    ...
    with mlflow.start_run(run_name="gradient_boosting"):
        ...
        logger.success("Gradient Boosting trained")
        ...
        for split_name, X, y in [("val", X_val, y_val), ("test", X_test, y_test)]:
            ...
            logger.info(f"Gradient Boosting | {split_name} | Acc: {acc:.4f} | F1: {f1:.4f}")
            ...
            ax.set_title(f"Gradient Boosting — {split_name}")
            plt.tight_layout()
            fig.savefig(out_dir / f"cm_gradient_boosting_{split_name}.png", dpi=150)
            plt.close(fig)
        ...
        pd.DataFrame([{"model": "Gradient Boosting", **results}]).to_csv(
            "reports/metrics_gradient_boosting.csv", index=False
        )
        logger.success(f"Gradient Boosting — Val Acc: {results['val_accuracy']} | Test Acc: {results['test_accuracy']}")
