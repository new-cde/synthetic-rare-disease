...
def run():
    ...
    with mlflow.start_run(run_name="random_forest"):
        ...
        for split_name, X, y in [("val", X_val, y_val), ("test", X_test, y_test)]:
            ...
            logger.info(f"Random Forest | {split_name} | Acc: {acc:.4f} | F1: {f1:.4f}")
            ...
            plt.tight_layout()
            fig.savefig(out_dir / f"cm_random_forest_{split_name}.png", dpi=150)
            plt.close(fig)
        ...
        pd.DataFrame([{"model": "Random Forest", **results}]).to_csv(
            "reports/metrics_random_forest.csv", index=False
        )
        logger.success(f"Random Forest — Val Acc: {results['val_accuracy']} | Test Acc: {results['test_accuracy']}")
